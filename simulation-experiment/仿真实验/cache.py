...
class AcrobotEnv(core.Env):
    ...
    **REFERENCE:**
    ...
    LINK_COM_POS_1 = 0.2792  #: [m] position of the center of mass of link 1
    LINK_COM_POS_2 = 0.4035  #: [m] position of the center of mass of link 2
    LINK_MOI1 = 0.1709  #: moments of inertia for both links
    LINK_MOI2= 0.0701
    g = 9.81
    ...
    def _step(self, a):
        ...
        #对称虚约束的方法
        # alpha=self.A+self.B*cos(s[1])
        # beta=-self.B*sin(s[1])
        # gamma=self.omega4*self.g*cos(self.a*s[1]+self.b)+self.omega5*self.g*cos((1+self.a)*s[1]+self.b)
        # I=((self.A+self.B*cos(s[1]))/(self.A+self.B*cos(self.theta2d)))**2
        ...
        # I2 = self.LINK_MOI
        # g = 9.81
        # Ed = -m1 * g * lc1 * cos(self.theta1d) - m2 * g * (l1 * cos(self.theta1d) + lc2 * cos(self.theta1d + self.theta2d))
        E = self.energy(s)
        ...
        #         np.cos(self.theta1d), self.LINK_LENGTH_1 * np.sin(self.theta1d)]
        #
        # g_p2 = [g_p1[0] - self.LINK_LENGTH_2 * np.cos(self.theta1d + self.theta2d),
        #         g_p1[1] + self.LINK_LENGTH_2 * np.sin(self.theta1d + self.theta2d)]
        #末端位置
        ...
        return (self._get_ob(), reward, False,[reward1,reward2,self.state,phi,Y,D,E] )
