...
class CriticNetwork:
	...
	def __init__(self,sess,state_dim,action_dim,load,pri):
		self.time_step = 0
		self.sess = sess
		self.pri=pri
		# create q network
	...
	###PER
	def train(self,y_batch,state_batch,action_batch,ISweights=None):
		self.time_step += 1
		if self.pri:
	...
	def per_train(self,y,state,action):
		abs_errors=self.sess.run(self.abs_errors,feed_dict={
													self.y_input:y,
	...
	def single_q_value(self,state_single,action_single):
		return self.sess.run(self.q_value_output,feed_dict={
			self.state_input:[state_single],
			self.action_input:[action_single]})
