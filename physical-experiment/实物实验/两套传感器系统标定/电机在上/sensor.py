...
class CONTROL(object):
    def __init__(self):
        ...
        # 使能控制
        self.sgn = lambda x: np.array([1,0], dtype=np.uint8) if x > 0 else np.array(
                                        [0,1], dtype=np.uint8) if x < 0 else np.array([0, 0], dtype=np.uint8)
        self.task0 = Task()
    ...
    def read_daq(self):
        ...
        self.angle_velocity2 = self.data3[0] *2* pi / 3  # 转换成弧度,电机极限转速为8000是使用2*pi/3
        # 杆2角度采集
